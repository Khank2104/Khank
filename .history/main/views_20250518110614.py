...
def generate_exercises(request):
    ...
    if goal and level and duration:
        ...
        try:
            ...
            suggested_exercises = ExerciseSuggestion.objects.filter(
                goal=goal,
                level=level,
                duration__lte=duration_int,
                bmi_min__lte=bmi,
                bmi_max__gte=bmi
            )[:4]
        except ValueError:
            pass  # Trường hợp không chuyển được duration -> int
    else:
        # Nếu không có query param, lấy từ SavedExercisePlan nếu có
        try:
            saved = SavedExercisePlan.objects.get(user=user)
            initial_data = {
                'goal': saved.goal,
                'level': saved.level,
    ...
    # Form luôn khởi tạo bằng initial_data
...
def save_exercise_plan(request):
    if request.method == 'POST':
        ...
        # Redirect kèm query params để tự load lại
        return HttpResponseRedirect(
            reverse('generate_exercises') + f'?goal={goal}&level={level}&duration={duration}'
        )
